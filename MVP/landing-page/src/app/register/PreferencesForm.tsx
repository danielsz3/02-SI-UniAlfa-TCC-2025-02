...
interface PreferencesFormProps {
  onNext: (data: PreferencesFormState) => void
  onBack: () => void
  defaultValues: PreferencesFormState
}
...
export default function PreferencesForm({ onNext, onBack, defaultValues }: PreferencesFormProps) {
  ...
  // Sincroniza o estado local com defaultValues ao montar ou quando defaultValues mudam
  useEffect(() => {
    setForm({
  ...
  return (
    <form
      className="w-full max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6 md:p-8 mt-6 md:mt-10"
      onSubmit={submit}
    >
